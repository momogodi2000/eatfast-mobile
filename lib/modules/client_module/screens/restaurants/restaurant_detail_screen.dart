...
import 'package:eatfast_mobile/shared/services/restaurant/restaurant_service.dart' as restaurant_service;
import 'package:eatfast_mobile/shared/services/restaurants/providers/restaurant_provider.dart';
import 'package:eatfast_mobile/modules/client_module/widgets/widgets/restaurants/menu_item_card.dart';
...
class _RestaurantDetailScreenState extends ConsumerState<RestaurantDetailScreen>
  ...
  Widget _buildRestaurantDetail(
    ...
    AsyncValue<List<restaurant_service.MenuCategory>> categoriesAsync,
  ...
  Widget _buildRestaurantInfo(Restaurant restaurant) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            (restaurant.description != null && restaurant.description!.isNotEmpty)
  ...
  Widget _buildMenuTab(AsyncValue<List<restaurant_service.MenuCategory>> categoriesAsync) {
  ...
  Widget _buildCategoryTabs(List<restaurant_service.MenuCategory> categories) {
    return Container(
      height: 50,
      color: DesignTokens.white,
        ...
        children: [
          _buildInfoSection('Adresse', [
            (restaurant.address != null)
                ? '${restaurant.address!.street}, ${restaurant.address!.city}${restaurant.address!.country != null ? ', ${restaurant.address!.country}' : ''}'
          ...
          _buildInfoSection('Contact', [
            restaurant.contact?.phone ?? 'Numéro de téléphone non disponible',
          ]),
          
          const SizedBox(height: DesignTokens.spaceXL),
          
          _buildInfoSection('Horaires', [
            (restaurant.openingHours != null && restaurant.openingHours!.isNotEmpty)
                ? restaurant.openingHours!.entries.map((e) => '${e.key}: ${e.value}').join(', ')
                : 'Horaires non disponibles'
          ]),
          
          const SizedBox(height: DesignTokens.spaceXL),
          
          _buildInfoSection('Spécialités', (restaurant.specialties != null && restaurant.specialties!.isNotEmpty)
              ? restaurant.specialties!
              : ['Information non disponible']),

          const SizedBox(height: DesignTokens.spaceXL),

          _buildInfoSection('Moyens de paiement', (restaurant.paymentMethods != null && restaurant.paymentMethods!.isNotEmpty)
