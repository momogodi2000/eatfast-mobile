...
class _RewardsCatalogScreenState extends ConsumerState<RewardsCatalogScreen> {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _RewardDetailsBottomSheet(
        ...
        onRedeem: () async {
          ...
          final request = LoyaltyRedemptionRequest(
          ...
          try {
            await ref.read(loyaltyProvider.notifier).redeemPoints(request);
  ...
  List<Widget> _buildActiveFilterChips(BuildContext context, RewardsFilterState filterState) {
    ...
    if (filterState.type != null) {
      chips.add(
        Chip(
          label: Text(filterState.type!.displayName),
    ...
    if (filterState.showOnlyAffordable) {
      chips.add(
        Chip(
          label: const Text('Seulement abordables'),
          deleteIcon: const Icon(Icons.close, size: 16),
          onDeleted: () {
            ref.read(rewardsFilterProvider.notifier).setShowOnlyAffordable(false);
          },
        ),
      );
    }
