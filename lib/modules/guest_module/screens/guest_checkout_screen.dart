...
import 'package:eatfast_mobile/shared/services/payment/presentation/widgets/enhanced_payment_selector.dart';
import 'package:eatfast_mobile/shared/services/payment/presentation/widgets/stripe_payment_widget.dart';
...
class _GuestCheckoutScreenState extends ConsumerState<GuestCheckoutScreen> {
  ...
  domain.PaymentMethod _selectedPaymentMethod = domain.PaymentMethod.cash;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Erreur d\'initialisation: $e')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Finaliser la commande'), elevation: 0),
      body: _isLoading
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  '${widget.subtotal.toStringAsFixed(0)} ${AppConfig.currencySymbol}',
                ),
              ],
            ),
            Row(
              ...
              children: [
                ...
                Text(
                  '${_deliveryFee.toStringAsFixed(0)} ${AppConfig.currencySymbol}',
                ),
              ],
            ),
          ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_selectedPaymentMethod == domain.PaymentMethod.stripe) ...[
              const SizedBox(height: 16),
              StripePaymentWidget(
                amount: widget.subtotal + _deliveryFee,
                currency: AppConfig.currency,
                orderId:
                    _currentSessionId ??
                    'temp-order', // Will be replaced with actual order ID
                onPaymentComplete: (bool success, String? error) {
                  if (success) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: _isLoading
            ...
            : Text(
                'Commander ${total.toStringAsFixed(0)} ${AppConfig.currencySymbol}',
              ),
      ),
    );
  }
  ...
  Future<void> _placeOrder(double total) async {
    ...
    try {
      final orderRequest = GuestOrderRequest(
        ...
        guestEmail: _emailController.text.isNotEmpty
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Num�ro de commande: ${orderData.orderNumber ?? orderData.orderId}',
              style: Theme.of(context).textTheme.titleMedium,
            ),
            const SizedBox(height: 24),
            Card(
                ...
                child: Column(
                  children: [
                    ...
                    ListTile(
                      ...
                      subtitle: Text(_formatPaymentMethod(paymentMethod)),
                    ),
                    if (orderData.totalAmount != null)
                      ListTile(
                        ...
                        subtitle: Text(
                          '${orderData.totalAmount!.toStringAsFixed(0)} ${orderData.currency ?? "FCFA"}',
                        ),
                      ),
                  ],
                ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () => Navigator.of(context).pushReplacement(
                  MaterialPageRoute(
                    builder: (context) =>
                        GuestOrderTrackingScreen(orderId: orderData.orderId),
                  ),
                ),
...
class GuestOrderTrackingScreen extends StatelessWidget {
  ...
  const GuestOrderTrackingScreen({super.key, required this.orderId});
