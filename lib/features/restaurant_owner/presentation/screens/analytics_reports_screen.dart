...
class _AnalyticsReportsScreenState extends ConsumerState<AnalyticsReportsScreen> {
  ...
  Widget _buildAnalyticsContent(RestaurantStats stats, AsyncValue<List<MenuItemPerformance>> itemPerformance) {
    return Column(
      ...
      children: [
        // Key Metrics Summary
        _buildSectionTitle('Résumé des Indicateurs Clés'),
        const SizedBox(height: DesignTokens.spaceMD),
        _buildKeyMetricsCards(stats),
        const SizedBox(height: DesignTokens.spaceXL),

        // Revenue Chart
        _buildSectionTitle('Évolution du Chiffre d\'Affaires'),
        const SizedBox(height: DesignTokens.spaceMD),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const Divider(),
            _buildStatRow('Complétées aujourd\'hui', stats.completedToday.toString()),
            const Divider(),
  ...
  Widget _buildItemPerformanceList(List<MenuItemPerformance> performance) {
    if (performance.isEmpty) {
      return _buildEmptyMessage('Aucune donnée de performance disponible');
    }
    ...
    return Column(
      children: [
        _buildInsightCard(
          ...
          description: 'Vos 5 articles les plus populaires génèrent 60% de vos revenus. Envisagez de promouvoir ces articles.',
          color: DesignTokens.warningColor,
        ),
        const SizedBox(height: DesignTokens.spaceMD),
        _buildInsightCard(
        ...
        _buildInsightCard(
          ...
          description: 'Votre note moyenne de ${stats.averageRating.toStringAsFixed(1)}/5 est excellente! Continuez comme ça.',
          color: DesignTokens.successColor,
        ),
      ],
    );
