...
import 'package:eatfast_mobile/shared/services/api/live_data_service.dart';
import 'package:eatfast_mobile/modules/guest_module/services/data/guest_service.dart';

class IntegrationTestHelper {
  static final IntegrationTestHelper _instance =
  ...
  Future<TestResult> testRealTimeFeatures() async {
    ...
    try {
      ...
      steps.add(
        await _testStep('Push Notification Init', () => Future.value(true)),
      );
  ...
  Future<TestResult> testApiConnectivity() async {
    ...
    try {
      ...
      steps.add(
        await _testStep('Backend Connectivity', () => _testBackendHealth()),
      );

      // Step 2: Test cache functionality
      steps.add(
        await _testStep('Cache Functionality', () => _testCacheSystem()),
      );
  ...
  Future<List<TestResult>> runAllTests() async {
    ...
    for (final result in results) {
      debugPrint(
        '  ${result.passed ? "✅" : "❌"} ${result.testName}: ${result.summary}',
      );
    }
  ...
  Future<bool> _testGuestOrderValidation() async {
    ...
    for (final test in validationTests) {
      ...
      final isValid =
          test['name'].toString().isNotEmpty &&
          test['phone'].toString().contains('+237') &&
          test['address'].toString().isNotEmpty;

      debugPrint(
        'Validation test: ${test['name']} - ${isValid ? "PASS" : "FAIL"}',
      );
    }
  ...
  Future<bool> _testPaymentGatewayConfig(String gateway) async {
    ...
    final configs = {'campay': true, 'noupay': true, 'stripe': true};
