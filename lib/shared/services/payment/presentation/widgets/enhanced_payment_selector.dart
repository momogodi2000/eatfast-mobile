...
import 'package:eatfast_mobile/shared/services/payment/domain/models/payment.dart' as domain;
import 'package:eatfast_mobile/shared/services/payment/data/unified_payment_service.dart' as service;
...
class EnhancedPaymentSelector extends StatefulWidget {
  ...
  @override
  State<EnhancedPaymentSelector> createState() =>
      _EnhancedPaymentSelectorState();
}

class _EnhancedPaymentSelectorState extends State<EnhancedPaymentSelector> {
  final service.UnifiedPaymentService _paymentService =
  ...
  Future<void> _loadPaymentMethods() async {
    try {
      ...
      setState(() {
        _paymentMethods = serviceMethods
            .map((m) => _convertPaymentMethod(m))
            .whereType<domain.PaymentMethod>()
            .toList();
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
        _paymentMethods =
            domain.PaymentMethod.values; // Fallback to all methods
        _isLoading = false;
      });
    }
  }

  domain.PaymentMethod? _convertPaymentMethod(
    service.PaymentMethod serviceMethod,
  ) {
...
class _PaymentProcessingWidgetState extends State<PaymentProcessingWidget> {
  ...
  service.PaymentMethod? _convertDomainToServiceMethod(
    domain.PaymentMethod domainMethod,
  ) {
    switch (domainMethod) {
      case domain.PaymentMethod.cash:
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<service.PaymentMethod>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData) {
          return const Center(
            child: Text('Impossible de charger les méthodes de paiement'),
          );
        }
