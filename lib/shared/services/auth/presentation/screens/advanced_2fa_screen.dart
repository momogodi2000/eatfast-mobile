...
import 'package:eatfast_mobile/shared/services/auth/domain/models/two_factor_auth.dart';
...
class _Advanced2FAScreenState extends ConsumerState<Advanced2FAScreen>
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          isScrollable: true,
          tabs: const [
            Tab(icon: Icon(Icons.security), text: 'Configuration'),
            Tab(icon: Icon(Icons.qr_code), text: 'TOTP'),
            Tab(icon: Icon(Icons.backup), text: 'Codes de secours'),
            Tab(icon: Icon(Icons.devices), text: 'Appareils'),
          ],
        ),
      ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text(
                      'Ne pas demander la 2FA sur cet appareil pendant 30 jours',
                    ),
                  ...
                  SwitchListTile(
                    title: const Text(
                      'Demander la 2FA pour les actions sensibles',
                    ),
                    subtitle: const Text(
  ...
  Widget _buildEnabledMethodsList(TwoFactorConfig config) {
    ...
    if (config.totpEnabled) {
      enabledMethods.add('Application d\'authentification');
    }
  ...
  Widget _buildBackupCodesTab(TwoFactorConfig config) {
    if (!config.isEnabled) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.backup_outlined, size: 80, color: Colors.grey),
            SizedBox(height: 16),
            Text(
    ...
    return BackupCodesWidget(backupCodes: config.backupCodes);
  }
  ...
  Widget _buildTrustedDevicesTab(TwoFactorConfig config) {
    ...
    return TrustedDevicesWidget(devices: config.trustedDevices);
  }
  ...
  void _enableTOTP() async {
    try {
      await ref
          ...
          .enableTotp(''); // TODO: Get verification code
  ...
  void _toggleRequireForLogin(bool value) async {
    try {
      await ref
          .read(twoFactorProvider.notifier)
          .updateSettings(requireForLogin: value);
    } catch (error) {
      _showErrorSnackBar(error.toString());
    }
  }
  ...
  void _toggleTrustedDevices(bool value) async {
    try {
      await ref
          .read(twoFactorProvider.notifier)
          .updateSettings(allowTrustedDevices: value);
    } catch (error) {
      _showErrorSnackBar(error.toString());
    }
  }
  ...
  void _toggleSensitiveActions(bool value) async {
    try {
      await ref
          .read(twoFactorProvider.notifier)
          .updateSettings(requireForSensitiveActions: value);
    } catch (error) {
      _showErrorSnackBar(error.toString());
    }
  }
