...
import 'package:eatfast_mobile/shared/services/auth/repositories/two_factor_repository.dart';
...
class TwoFactorNotifier extends StateNotifier<TwoFactorState> {
  ...
  Future<TwoFactorSetupResponse> setupTotp() async {
    try {
      ...
      state = state.copyWith(setupResponse: response, isLoading: false);
      return response;
    } catch (error) {
      state = state.copyWith(isLoading: false, error: error.toString());
      rethrow;
  ...
  Future<void> generateBackupCodes() async {
    try {
      ...
      state = state.copyWith(newBackupCodes: codes, isLoading: false);
      await loadConfig();
    } catch (error) {
      state = state.copyWith(isLoading: false, error: error.toString());
      rethrow;
