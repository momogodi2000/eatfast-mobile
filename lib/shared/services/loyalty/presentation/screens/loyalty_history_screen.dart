...
class _LoyaltyHistoryScreenState extends ConsumerState<LoyaltyHistoryScreen>
  ...
  Widget _buildActivityByTypeChart(List<LoyaltyTransaction> transactions) {
    ...
    final totalTransactions = typeCounts.values.fold<int>(
      0,
      (sum, count) => sum + count,
    );
    ...
    return Container(
      ...
      child: Column(
        children: recentTransactions
            .map(
              (transaction) =>
                  _SummaryTransactionTile(transaction: transaction),
            )
            .toList(),
      ),
    );
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              ...LoyaltyTransactionType.values.map(
                (type) => RadioListTile<LoyaltyTransactionType?>(
