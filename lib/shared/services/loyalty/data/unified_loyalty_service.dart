...
import '../domain/models/loyalty.dart' as base;
import '../domain/models/enhanced_loyalty_models.dart' as enhanced;
import 'package:eatfast_mobile/shared/models/loyalty_models.dart' as shared;
...
class UnifiedLoyaltyService {
  ...
  Future<LoyaltyResponse> getLoyaltyProgram() async {
    try {
      ...
      if (response.statusCode == 200) {
        final loyaltyProgram = base.LoyaltyProgram.fromJson(
          response.data['data'],
        );
  ...
  Future<enhanced.RewardEligibility> checkRewardEligibility(
    String rewardId,
  ) async {
    try {
      final response = await _apiClient.get(
  ...
  Future<List<enhanced.LoyaltyTransaction>> getLoyaltyHistory({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data
            .map((item) => enhanced.LoyaltyTransaction.fromJson(item))
            .toList();
      } else {
        throw Exception(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return TierInfoResponse(
          tiers: shared.LoyaltyTier.values,
          currentTier: _parseTier(data['currentTier']),
        ...
        return TierInfoResponse(
          tiers: shared.LoyaltyTier.values,
          currentTier: shared.LoyaltyTier.bronze,
          tierBenefits: {},
          success: false,
  ...
  shared.LoyaltyTier _parseTier(String? tierName) {
    if (tierName == null) return shared.LoyaltyTier.bronze;
    switch (tierName.toLowerCase()) {
      case 'bronze':
        return shared.LoyaltyTier.bronze;
      case 'silver':
        return shared.LoyaltyTier.silver;
      case 'gold':
        return shared.LoyaltyTier.gold;
      case 'platinum':
        return shared.LoyaltyTier.platinum;
      case 'diamond':
        return shared.LoyaltyTier.diamond;
      default:
        return shared.LoyaltyTier.bronze;
    }
  }
  ...
  Future<List<enhanced.EarningOpportunity>> getEarningOpportunities() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data
            .map((item) => enhanced.EarningOpportunity.fromJson(item))
            .toList();
      } else {
        throw Exception(
  ...
  Future<List<enhanced.LoyaltyRecommendation>>
  ...
  Future<List<enhanced.SpecialEvent>> getActiveSpecialEvents() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return data
            .map((item) => enhanced.SpecialEvent.fromJson(item))
            .toList();
      } else {
...
class TierInfoResponse {
  final List<shared.LoyaltyTier> tiers;
  final shared.LoyaltyTier currentTier;
