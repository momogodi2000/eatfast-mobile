...
class SocialSharingService {
  ...
  Future<ShareResult> _executeShare(ShareContent content, SocialPlatform platform) async {
    try {
  ...
  Future<void> _shareToWhatsApp(ShareContent content) async {
    ...
    if (content.imageData != null) {
      ...
      final tempFile = await _saveImageToTemp(content.imageData!, 'whatsapp_share.png');
  ...
  Future<void> _shareToInstagram(ShareContent content) async {
    if (content.imageData != null) {
      final tempFile = await _saveImageToTemp(content.imageData!, 'instagram_share.png');
      // ignore: deprecated_member_use
  ...
  Future<void> _shareToLinkedIn(ShareContent content) async {
    final linkedinUrl = 'https://www.linkedin.com/sharing/share-offsite/?url=${Uri.encodeComponent(content.url)}';
  ...
  Future<void> _shareToTelegram(ShareContent content) async {
    final text = '${content.title}\n\n${content.message}\n\n${content.url}';
    final telegramUrl = 'https://t.me/share/url?url=${Uri.encodeComponent(content.url)}&text=${Uri.encodeComponent(text)}';
  ...
  Future<void> _shareViaEmail(ShareContent content) async {
    final emailUrl = 'mailto:?subject=${Uri.encodeComponent(content.title)}&body=${Uri.encodeComponent('${content.message}\n\n${content.url}')}';
  ...
  Future<void> _shareGeneric(ShareContent content) async {
    ...
    if (content.imageData != null) {
      final tempFile = await _saveImageToTemp(content.imageData!, 'share_image.png');
      // ignore: deprecated_member_use
      await Share.shareXFiles(
        [XFile(tempFile.path)],
        text: text,
  ...
  Future<String> _getReferralCode() async {
    try {
      final response = await _apiClient.get('${ApiConstants.socialReferralCreate}/my-code');
  ...
  Future<Map<String, dynamic>> _getReferralData() async {
    try {
      final response = await _apiClient.get('${ApiConstants.socialReferralCreate}/program-info');
  ...
  /// Track share event with backend
  Future<String> _trackShareEvent(ShareContent content, SocialPlatform platform) async {
    try {
