...
class PaymentManager {
  ...
  Future<PaymentResult> processPayment(PaymentRequest request) async {
    ...
    debugPrint('[PaymentManager] Processing payment for order ${request.orderId}');
    debugPrint('[PaymentManager] Preferred provider: ${request.preferredProvider.displayName}');
  ...
  Future<PaymentResult> _processMobileMoneyWithFallback(PaymentRequest request) async {
  ...
  Future<PaymentStatusResponse> _pollPaymentStatus({
    ...
    for (int attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        debugPrint('[PaymentManager] Payment status (attempt ${attempt + 1}/$maxAttempts): ${status.status.displayName}');
      } catch (e) {
  ...
  void _validateRequest(PaymentRequest request) {
    ...
    if (request.preferredProvider == PaymentProvider.mtnMomo ||
      ...
      if (request.phoneNumber == null || request.phoneNumber!.isEmpty) {
        throw PaymentValidationException('Phone number is required for mobile money payments');
      }
    }

    // Validate Stripe requirements
    if (request.preferredProvider == PaymentProvider.stripe) {
      if (request.stripePaymentMethodId == null || request.stripePaymentMethodId!.isEmpty) {
        throw PaymentValidationException('Payment method is required for card payments');
      }
    }
  }
